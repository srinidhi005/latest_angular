...
export class ComparatorComponent implements OnInit {
  ...
  async loadCompany(type, srcObj, index) {
    ...
    try {
      ...
      loadedCompScenarios = scenarios[srcObj.companyname];
    } catch (error) {
  ...
  async loadComparator() {
    ...
    try {
      const scenarios: any = await this.apiService.getData(
        this.urlConfig.getScenarioAPI() + [
          this.selectedCompanyOne.companyname,
          this.selectedCompanyTwo.companyname,
          this.selectedCompanyThree.companyname,
          this.selectedCompanyFour.companyname
        ].join()).toPromise();
      this.scenariosForCompanyOne = scenarios[this.selectedCompanyOne.companyname];
      this.scenariosForCompanyTwo = scenarios[this.selectedCompanyTwo.companyname];
      this.scenariosForCompanyThree = scenarios[this.selectedCompanyThree.companyname];
      this.scenariosForCompanyFour = scenarios[this.selectedCompanyFour.companyname];
    } catch (error) {

    }

    try {
      const bmActualsOne = await this.apiService.getData(this.urlConfig.getBenchmarkingActualsAPI() + [
        this.selectedCompanyOne.companyname,
        this.selectedCompanyTwo.companyname,
        this.selectedCompanyThree.companyname,
        this.selectedCompanyFour.companyname
      ...
      this.benchmarkingActualsTwo = bmActualsOne[1];
      this.benchmarkingActualsThree = bmActualsOne[2];
      this.benchmarkingActualsFour = bmActualsOne[3];
    } catch (error) {
  ...
  exportToPDF() {
    ...
    finalArray.push(
      ...headers, revCagrArray, ebitdaMarginArray, netIncomeMarginArray,
      OROAArray, ROAArray, ROTCArray, ROEArray, CRArray, QRArray,
      CashRArray, SRArray, DEArray, DAArray, debtToEbitdaArray,
      ICRArray, TDEArray, DSCRArray);
  ...
  getMappedArr(inputArr, suffix) {
    return inputArr.map((value: any, index) => {
      if (index === 0) {
        return {
          text: value,
          margin: [0, 10, 0, 10],
