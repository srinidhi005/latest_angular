...
export class RatiosComponent implements OnInit, OnDestroy {
  ...
  ratiosLoaded = false;
  constructor(
    private rmiApiService: RMIAPIsService,
  ...
  ngOnDestroy() {
    ...
    if (this.RatioScenarionSubscription) {
      this.RatioScenarionSubscription.unsubscribe();
    }
  }
  ...
  exportToPDF() {
    ...
    const returnRatiosKeys = ['name']
      .concat(onlyYears)
      .concat(keysReturn.filter((k) => k.indexOf('-') >= 0));
    ...
    const finalDataForReturn = [cloneDeep(commmonHeaders)]
      .concat([returnRatiosHeaders])
      .concat(returnRatiosData);

    // ---------------------------------------------------------------------------------------------------
    //profitability Ratios
    const keysProfit = Object.keys(
      this.excelService.profitabilityRatiosData[0]
    );
    onlyYears = keysReturn.filter((k) => {
      return k.indexOf('-') < 0 && k != 'name';
    });

    const profitabilityRatiosKeys = ['name']
      .concat(onlyYears)
      .concat(keysProfit.filter((k) => k.indexOf('-') >= 0));
    const profitabilityRatiosHeaders = profitabilityRatiosKeys.map(
      (name, index) => {
        if (index == 0) {
          return {
    ...
    const finalDataForProfit = [cloneDeep(commmonHeaders)]
      .concat([profitabilityRatiosHeaders])
      .concat(profitabilityRatiosData);
    ...
    const liquidityRatiosKeys = ['name']
      .concat(onlyYears)
      .concat(keysLiquidity.filter((k) => k.indexOf('-') >= 0));
    ...
    const finalDataForLiquidity = [cloneDeep(commmonHeaders)]
      .concat([liquidityRatiosHeaders])
      .concat(liquidityRatiosData);
    ...
    const solvencyRatiosKeys = ['name']
      .concat(onlyYears)
      .concat(keysSolvency.filter((k) => k.indexOf('-') >= 0));
    ...
    const finalDataForSolvency = [cloneDeep(commmonHeaders)]
      .concat([solvencyRatiosHeaders])
      .concat(solvencyRatiosData);
