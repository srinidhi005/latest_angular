...
export class HeaderComponent implements OnInit {
  ...
  constructor(
   ...
   ngOnInit() {
	  if(this.auth.authServiceLoaded){
      ...
      this.auth.getUserById(this.loggedInUserId).subscribe(
        (res) => {
  ...
  confirmLogout(){
    ...
    const dialogRef = this.excelService.showConfirmMessage(message, "Confirm", "Cancel", '400px', '120px');
